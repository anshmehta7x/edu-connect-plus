...
document.addEventListener('DOMContentLoaded', function(){
    ...
    .then(data => {
        ...
        items.forEach(item => {
            ...
            //access image tag inside div
            console.assert('YOOOOOOOOO');
            const img = item.querySelector("img");
            img.src = item.id + ".jpg";

            item.addEventListener("click", () => {
                console.assert("AMONGS");
                window.location.href = "userprofile.html" + "?targetid=" + encodeURIComponent(item.id);
            });    
        });
